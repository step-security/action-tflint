...
echo '::group::Preparing'
  ...
  if [[ -z "${INPUT_TFLINT_VERSION}" ]] || [[ "${INPUT_TFLINT_VERSION}" == "latest" ]]; then
    ...
    # Only send GITHUB_TOKEN to api.github.com when running on github.com; a GHES-issued token would 401.
    if [[ "${GITHUB_SERVER_URL:-https://github.com}" == "https://github.com" ]]; then
      AUTH=(-H "Authorization: Bearer ${INPUT_GITHUB_TOKEN}")
    else
      AUTH=()
    fi
    tflint_version=$(curl "${AUTH[@]}" --silent --show-error --fail --location "https://api.github.com/repos/terraform-linters/tflint/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
  else
